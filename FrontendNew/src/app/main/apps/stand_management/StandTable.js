...
import StandTablePaginationActions from './StandTablePaginationActions';
...
const EnhancedTable = ({ columns, data, onRowClick }) => {
	...
	const handleChangePage = (stand, newPage) => {
		gotoPage(newPage);
	};

	const handleChangeRowsPerPage = stand => {
		setPageSize(Number(stand.target.value));
	};
	...
	return (
		<div className="flex flex-col min-h-full sm:border-1 sm:rounded-16 overflow-hidden">
			...
			<TablePagination
				...
				ActionsComponent={StandTablePaginationActions}
			/>
		</div>
	);
};
